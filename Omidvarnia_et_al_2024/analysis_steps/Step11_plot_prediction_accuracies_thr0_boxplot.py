"""
This script loads subject-specific results and plots group-mean boxplots of
the zero tSNR treshold (i.e., all ROIs in the feature vectors) and at the 
maximum population size.

Usage example:
    python Step11_plot_prediction_accuracies_thr0_boxplot.py \
...
feature_names2.append('Confounds')
...
for target_no in range(N_targets):
    ...
    # ---------------------------
    # Get the colors from this plot
    # ---------------------------
    colors = []
    fig = plt.figure(1)
    ax = fig.add_subplot(111)
    im = ax.plot(accuracy_subjs_vs_ROIs1.T, linewidth=4)
    for n_curves in range(N_features): # Get the curve colors
        colors.append(im[n_curves].get_color())

    im = ax.plot(accuracy_subjs_vs_ROIs4, 'k', linewidth=4)
    colors.append(im[0].get_color()) # Add black color
    plt.close()
    
    # ---------------------------
    # Figure 2 - Boxplot: No Confound removal
    # ---------------------------
    fig_filename = os.path.join(figures_folder,
        f'Figure2_NoConfoundRemoved_target{target_no}_boxplot.png')

    fig = plt.figure(1, figsize=(25, 25))
    ax = fig.add_subplot(111)
    
    tmp = np.concatenate(
        (accuracy_subjs_vs_ROIs1, accuracy_subjs_vs_ROIs4.T), 
        axis=0
    ).T

    im = ax.boxplot(
        tmp, 
        labels=feature_names2,
        patch_artist=True,  # fill with color
    )
    # fill with colors
    for patch, color in zip(im['boxes'], colors):
        patch.set_facecolor(color)
    # Customize the boxplot components
    for box in im['boxes']:
        box.set_linewidth(12)  # Adjust the linewidth for the boxes

    for whisker in im['whiskers']:
        whisker.set_linewidth(12)  # Adjust the linewidth for the whiskers
    
    
    ax.set_ylabel('Accuracy', fontweight='bold', fontsize=80)
    ax.set_title(f'{target_labels2[target_no]}\n', fontweight='bold', fontsize=130)
    ax.set_xticklabels([])
    # ax.legend(feature_names2, loc='center left', bbox_to_anchor=(1,.5))
    # ax.set_aspect('equal')
    ...
    for ytick in yticks:
        ax.axhline(ytick, linestyle='dashed', color='gray', alpha=0.8, linewidth=12)
    ...
    # ---------------------------
    # Figure3 - Boxplot: Confound removal
    # ---------------------------
    fig_filename = os.path.join(figures_folder,
        f'Figure3_ConfoundRemoved_target{target_no}_boxplot.png')
        
    fig = plt.figure(1, figsize=(25, 25))
    ax = fig.add_subplot(111)
    
    tmp = np.concatenate(
        (accuracy_subjs_vs_ROIs2, accuracy_subjs_vs_ROIs4.T), 
        axis=0
    ).T
    ...
    # ---------------------------
    # Figure 4 - Boxplot: Combined features with confounds
    # ---------------------------
    fig_filename = os.path.join(figures_folder,
        f'Figure4_CombinedFeatures_target{target_no}_boxplot.png')

    fig = plt.figure(1, figsize=(25, 25))
    ax = fig.add_subplot(111)
    
    tmp = np.concatenate(
        (accuracy_subjs_vs_ROIs3, accuracy_subjs_vs_ROIs4.T), 
        axis=0
    ).T
