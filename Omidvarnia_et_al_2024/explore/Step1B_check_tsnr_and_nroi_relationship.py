...
Example usage:
    python Step1B_check_tsnr_and_nroi_relationship.py --model_type rf \
...
tsnr_vec = np.arange(0, 1, .05)

N_thresh = len(tsnr_vec)
...
if not os.path.isfile(accuracy_out_filename):
    ...
    # ------------------------------------------
    for n_tsnr in range(N_thresh):

        val_tsnr = tsnr_vec[n_tsnr]
        print(f"tSNR threshold %{val_tsnr} ...")
        ...
        # -------------------------------------------------
        brain_map_df_thresholded = complexity_analysis.thresh_by_tSNR_at_groupLevel(
        ...
        accuracy_all[n_tsnr] = mean_score
        scores_all[n_tsnr] = scores
        best_alpha_all[n_tsnr] = best_param
        N_ROI_supraThresh_all[n_tsnr] = val_tsnr

        # -------------------------------------------------
        # Save the results along the way
        # -------------------------------------------------
        key1 = f'model_trained_thresh{n_tsnr}'
        value1 = model_trained
        key2 = f'inspector_thresh{n_tsnr}'
        value2 = inspector
        ...
        print(
            (f"Accuracy for {feature_name} at the "
             f"tSNR thresholding at %{(val_tsnr)} was analysed!")
            )
