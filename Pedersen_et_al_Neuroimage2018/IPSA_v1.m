function IPSA = IPSA_v1(rsfMRI_2D)
% Instantaneous phase synchrony analysis (IPSA) for estimating time-resolved fMRI connectivity: 
%
% Input:     rsfMRI_2D,      2D narrow band-pass filtered fMRI data (nodes x time-points, or NxT).
%
% Output:    ISPA,           3D IPSA (NxNxT) - NB! high storage capacity needed with many nodes and/or time-points.
%
% Reference: Pedersen, Omidvarnia, Zalesky & Jackson (2017), bioRxiv, 179820

fprintf('\n\t Instantaneous phase synchrony analysis (IPSA):\n')
reverseStr = '';
theElapsedTime = tic;

IPSA = zeros(size(rsfMRI_2D,1),size(rsfMRI_2D,1),size(rsfMRI_2D,2)); % pre-allocate 3D IPSA matrix
